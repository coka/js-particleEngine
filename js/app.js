...
var ParticleController = function (x, y) { this.particles = []; }
...
ParticleController.prototype.addParticles = function (x, y, n, mousev)
{
  for (var i = 0; i < n; i++)
  {
    ...
    this.particles.push(new Particle(x + randVec[0] * Math.random() * 15.0,
...
ParticleController.prototype.update = function ()
{
  ...
  {
    if (this.particles[i].isDead) { this.particles.splice(i, 1); i--; }
...
window.onload = function execute()
{
  ...
  function draw()
  {
    ...
    {
      graphics.drawCircle(controller.particles[i].loc[0],
                          controller.particles[i].loc[1],
                          controller.particles[i].radius);
    }
  }

  function update() { controller.update(); }
}
